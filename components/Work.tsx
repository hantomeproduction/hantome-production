...
const ProjectCard: React.FC<ProjectProps> = ({ id, title, customThumbnail }) => {
  return (
    <div onClick={() => window.open(`https://www.youtube.com/watch?v=${id}`, '_blank')} className="group/card cursor-pointer flex flex-col gap-2 w-[220px] md:w-[280px] shrink-0 snap-start relative">
        ...
        <div className="flex flex-col gap-1 border-t border-white/5 pt-2 pl-1">
            <div className="flex justify-between items-start gap-2">
                <h3 className="text-gray-200 font-medium text-[11px] md:text-[13px] leading-snug line-clamp-2 group-hover/card:text-white transition-colors duration-300 tracking-tight">{title}</h3>
                <ArrowUpRight className="w-3 h-3 text-gray-600 group-hover/card:text-white transition-colors duration-300 shrink-0" />
            </div>
            <div className="flex items-center mt-1">
                <span className="text-[8px] font-mono text-green-500 uppercase tracking-widest bg-green-500/10 px-2 py-0.5 rounded-sm">YOUTUBE</span>
            </div>
        </div>
    </div>
  );
    ...
    return (
        <div className="w-full py-2 md:py-3 relative flex flex-col justify-center">
            ...
            <div className="relative group w-full flex-1 flex flex-col justify-center">
                ...
                <div ref={scrollRef} className="flex overflow-x-auto gap-4 px-6 md:px-0 pb-3 snap-x snap-mandatory no-scrollbar scroll-smooth w-full" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
...
export const Work: React.FC = () => {
  ...
  return (
        <section id="work" className="w-full h-[100dvh] snap-start snap-always shrink-0 bg-[#050505] flex flex-col overflow-hidden relative border-b border-white/5">
             ...
             <div className="flex-1 flex flex-col justify-center max-w-[1920px] mx-auto w-full md:px-6 py-2 md:py-4 overflow-hidden">
                 <Reveal>
                    <div className="mb-2 md:mb-4 px-6 md:px-0 pb-2 md:pb-4 border-b border-white/5 relative shrink-0">
                         <h2 className="text-[7vw] md:text-[5vw] leading-[0.8] font-bold tracking-tighter text-white uppercase opacity-90">PORTFOLIO</h2>
                    </div>
                </Reveal>
                 <div className="flex-1 flex flex-col justify-evenly gap-1 overflow-hidden">
                    {/* ✨ 여기서 title이랑 subtitle 순서를 스왑했어! */}
